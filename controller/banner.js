...
data.create = async function (req, res) {
    ...
    const result = await pool.query('INSERT INTO banner SET  bannerImage_name = ?, created_at= ?'
        , [bannerImage_name, created_at]);
...
data.dataTable = async function (req, res) {
    ...
    const totalbanners = await pool.query('SELECT COUNT(*) as count FROM banner WHERE is_deleted = 0');
    let bannerList = await pool.query('SELECT * FROM banner WHERE is_deleted = 0');
    bannerList = bannerList[0];
    let result = [];
    if (bannerList.length > 0) {
        bannerList.forEach(function (data) {
            let value = [];
            const bannerImage = "<img src='/bannerImages/" + data.bannerImage_name + " ' class='thumbnail' style='height:220px;width:100%'>"
            value.push(bannerImage);
            let action = "<a href='/banner/edit/" + data.id + "' class='btn btn-warning btn-xs'><i class='fa fa-fw fa-edit'></i></a> &nbsp;";
            action += "<a href='javascript:deleteBanner(" + data.id + ")' class='btn btn-danger btn-xs'><i class='fa fa-trash'></i></a> &nbsp;";
...
data.editForm = async function (req, res) {
    ...
    return res.render('pages/banner/edit', { sidebar: 'banner', banner: banner[0] });
}
...
data.edit = async function (req, res) {
    ...
    await pool.query('UPDATE banner SET bannerImage_name = ? WHERE id = ?'
        , [bannerImage_name, id]);
